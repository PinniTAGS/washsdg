...
// Check for force canvaskit in URL parameters
const search_params = new URLSearchParams(window.location.search);
const force_canvaskit = search_params.get('force_canvaskit') === 'true';
const user_config = force_canvaskit ? {'renderer': 'canvaskit'} : {};

_flutter.loader.load({
  config: user_config,
  serviceWorkerSettings: {
    serviceWorkerVersion: "2102572569",
  },
  onEntrypointLoaded: async function(engine_initializer) {
    console.log('Flutter entrypoint loaded');

    // Initialize and run the app
    const app_runner = await engine_initializer.initializeEngine();
    console.log('Running app');
    await app_runner.runApp();

    // Hide the splash screen
    var splash_element = document.getElementById('loading-screen');
    if (splash_element) {
      console.log('Hiding splash screen');
      splash_element.style.display = 'none';
    } else {
      console.error('Could not find splash element');
    }
  }
});

// Update the loading percentage every second
let loading_interval = setInterval(function() {
  load_percentage += load_increment;
  if (load_percentage > 100) {
    load_percentage = 100;
    clearInterval(loading_interval);
  }
  document.getElementById('loading-percentage').innerText = load_percentage + '%';
}, 1000);
